...
This program records the intensities at user-defined wavelengths.
Users can set the integration time and wavelength(s) of interest.
The program will then return the intensities at those points.
...
def prompt_user_for_pixel_selection():
    ...
    while True:
        try:
            ...
            pixels_input = input(
                "Enter up to 4 pixel (wavelength) positions to focus on, separated by commas (e.g., 100,200,300,400): ")
            # Convert an input string into a list of integers
    ...
    if device_ids:
        ...
        try:
            ...
            try:
                ...
                print(f"[START] Reading spectra for dev S/N = {sn}",
                      flush=True)  # Indicate the start of spectra reading
                ...
                print(
                    f"Error during device operation: {e.get_error_details()}")  # Print any errors encountered during the device operation

        except OceanDirectError as e:
            print(
                f"Error opening device: {e.get_error_details()}")  # Print any errors encountered during device opening

        # Delete the device instance and shut down OceanDirect only after the user presses Enter
        print("**** Complete ****")  # Print exit message
        input("Press Enter to close...")  # Wait for the user to press Enter
