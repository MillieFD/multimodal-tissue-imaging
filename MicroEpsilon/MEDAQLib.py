...
if os.name == "nt":
    # For Windows, the WinDLL ctype is required
...
class SENSOR_TYPE(enum.IntEnum):
    ...
    NO_SENSOR = 0  # Only for internal use
    ...
    SENSOR_IFC2410 = 64  # confocalDT
    SENSOR_IFC2411 = 65  # confocalDT
    SENSOR_IFC2415 = 66  # confocalDT
    SENSOR_IFC2416 = 67  # confocalDT
    SENSOR_IFC2421 = 46  # confocalDT
    SENSOR_IFC2422 = 47  # confocalDT
    SENSOR_IFC2445 = 39  # confocalDT
    SENSOR_IFC2451 = 30  # confocalDT
    SENSOR_IFC2461 = 44  # confocalDT
    SENSOR_IFC2465 = 62  # confocalDT
    SENSOR_IFC2466 = 63  # confocalDT
    SENSOR_IFC2471 = 26  # confocalDT
...
class MEDAQLib:
    """load MEDAQLib.dll or MEDAQLib.so from the module path"""
    ...
    def GetDLLVersion(self):
        ...
        The version is stored in `versionStr` and is limited to the length of `maxLen` (should be at least 64 bytes)
    ...
    def SetParameterInt(self, paramName, paramValue):
        """Specifies a 4-Byte integer parameter for an `OpenSensor` or `SensorCommand` function call.
    ...
    def SetParameterDouble(self, paramName, paramValue):
        """Specifies an 8-Byte double parameter for an `OpenSensor` or `SensorCommand` function call.
    ...
    def SetParameterString(self, paramName, paramValue):
        """Specifies a string parameter for an `OpenSensor` or `SensorCommand` function call.
    ...
    def SetParameterBinary(self, paramName, paramValue):
        """Specifies a binary data array parameter for an `OpenSensor` or `SensorCommand` function call.
    ...
    def SetParameters(self, parameter_list):
        ...
        :return: None
        """
        self._last_error = self._set_parameters_org(
            self.iSensor, bytes(parameter_list, "utf8")
        )

    # tested Ok.
    def GetParameterInt(self, paramName):
        """Returns a 4-Byte integer parameter after calling `SensorCommand`.
    ...
    def GetParameterString(self, paramName, max_len=0, sensor_encoding="cp1252"):
        ...
        if len.value == 0:
            ...
            self._get_parameter_string_org(
                self.iSensor, bytes(paramName, "utf8"), c_char_p(), byref(len)
            )
    ...
    def GetParameters(self, max_len=0, sensor_encoding="cp1252"):
        ...
        if len.value == 0:
            ...
            self._last_error = self._get_parameters_org(
                self.iSensor, c_char_p(), byref(len)
            )
        param_value = create_string_buffer(len.value)
        self._last_error = self._get_parameters_org(
            self.iSensor, param_value, byref(len)
        )
    ...
    def EnableLogging(
        ...
        :param enableLogging: This parameter enables or disables logging to a file
        ...
        :param logFile: File name for the generated log file.
        :param logAppend: This parameter specifies if the logfile should be cleared at
                        opening or if the new data should be appended to an existing logfile.
        :param logFlush: This parameter specifies if the logfile should be flushed
                        after each output.
        :param logSplitSize: If this parameter is greater than 0, the logfile is closed and
    ...
    def _transferdata_helper(self, maxValues, timeStampEnable):
        ...
        :return: a tuple with the raw data, the scaled data and the real number of transfered data values
